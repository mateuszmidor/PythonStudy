...
def newProfit(when: datetime, what="PHYS", paid_pln: int = 0, received_pln: int = 0) -> ProfitPLN:
    ...
    buy_sell_pln = BuySellPairPLN(
        ...
        sell_pln_quotation_date=when.date() - timedelta(days=1),
...
def newTax(when: datetime, paid_pln: int = 0) -> TaxItemPLN:
    ...
    return TaxItemPLN(source=tax, tax_pln_quotation_date=when.date() - timedelta(days=1), paid_tax_pln=paid_pln)
...
def newDividend(when: datetime, dividend_pln: int = 0, tax_pln: int = 0) -> DividendItemPLN:
    ...
    return DividendItemPLN(
        source=dividend,
        dividend_pln_quotation_date=when.date() - timedelta(days=1),
        received_dividend_pln=dividend_pln,
        tax_pln_quotation_date=when.date() - timedelta(days=1),
